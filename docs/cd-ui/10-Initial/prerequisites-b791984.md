...
[Manage Server Identities](../20-Configuration/manage-server-identities-1c4cbe6.md "A server identity is a list of custom domain SANs (Subject Alternative Names) and one or two server certificates that share the list of SANs. Create a new server certificate and a certificate signing request (CSR) to obtain a certificate for your domains from a trusted certificate authority. Use the secondary server certificate to rotate to a new one with an extended lifetime, or from a different CA. Use the rotation REST API to integrate with your on-premise or cloud-based automation, like ACME CERTBOT or other solutions that are able to handle CSR (PKCS#10 PEM) and certificate chains (set of X.509 PEM).")
